def distPacc(mirror, deltaPacc):
    ...
    Attempt at distributing to governed gens only... -> Doesn't seem to work
    """
    ...
    while tol_Flag:
        ...
        for c_gen in mirror.Machines:
            ...
            if (not c_gen.cv['IRPflag']) and (iteration == 1):
                # generator doesn't participate in inertia response, but does get slack errors
                continue

            # Ensure off generators don't supply power
            if c_gen.cv['St'] == 0:
                c_gen.cv['Pe'] = 0
                c_gen.cv['Q'] = 0
                c_gen.setPvals()
            ...
            # Handle global Slack Pacc
            if c_gen.globalSlack:
                if iteration == 1:
                    #distribute to slack on First pass 
                    c_gen.cv['Pe'] = c_gen.cv['Pe'] - Pacc * (c_gen.H/Hgov)
                    # Set Pe_calc
                    c_gen.cv['Pe_calc'] = c_gen.cv['Pe'] 
                    # remove slack inertia from system sum
                    Hss -= c_gen.H
                else:
            ...
            #Distribute delta Pacc to non slack other gens
                ...
                if iteration == 1:
                    # Only distribute to governed gens on first pass
                    c_gen.cv['Pe'] = c_gen.cv['Pe'] - Pacc * (c_gen.H/Hgov)
                else:
                    # Distribute slack error to all gens.
        ...
        toc1 = time.time()
        # Pe is distributed to all participating generators in all areas, solve Power flow
