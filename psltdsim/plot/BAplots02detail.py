def BAplots02detail(mirror, xAxisList, blkFlag=True, printFigs=False):
    ...
    for BA in mir.BA:
        ...
        ax.set_xlim(xAxisList[0],xAxisList[1])
        ...
        if printFigs: plt.savefig(caseName+BA.name+'Detail.pdf', dpi=300)
    ...
    fig.tight_layout()
    plt.show(block=False)
    if printFigs: plt.savefig(caseName+'RACEdetail'+'.pdf', dpi=300)
    plt.pause(0.00001)
    ...
    fig.set_size_inches(9/mini, 2.5)
    fig.tight_layout()
    if printFigs: plt.savefig(caseName+'FreqDetail'+'.pdf', dpi=300)
