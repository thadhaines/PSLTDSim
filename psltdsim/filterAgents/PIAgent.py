class PIAgent():
    ...
    def __init__(self, mirror, parentAgent, kp, a, initVal = 0.0):
        self.mirror = mirror
        self.parentAgent = parentAgent # mirror agent that uses this filter
        self.kp = kp
        # Note: a = Ki/Kp and sets the placement of the zero in the PI controller
        self.a = a
        ...
        # Dynamic init
        self.sys1 = sig.StateSpace([0.0],[1.0],[self.kp*self.a],[self.kp])
        ...
        if mirror.debug:
            print("*** PI added to %s" % (self.parentAgent))
