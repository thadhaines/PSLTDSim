...
for area = 1:max(size(mir.areaN)) % for each area
    ...
    for slack = 1:max(size(mir.(curArea).slackBusN))
        ...
        psdsDataNdx = intersect(jfind(psds_data, mir.(curArea).(curSlack).BusName),pg_col);
    ...
    for gen = 1:max(size(uniueGens))
        ...
        for GenId = 1:mir.(curArea).(curGenBus).Ngen
            ...
            % find psds data by bus name
            psdsDataNdx = intersect(jfind(psds_data, mir.(curArea).(curGenBus).BusName),pg_col);
            if max(size(psdsDataNdx))>1
                % check for multiple intersect, search by bus number
                if debug disp(name); end
    ...
    grid on
    if noCase ==1
        title('Deviation of LTD Real Power Output from PSDS')
    else
        title({'Deviation of LTD Real Power Output from PSDS'; ['Case: ', LTDCaseName]})
    end
