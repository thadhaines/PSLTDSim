function [  ] = compareQbr2( mir, psds_data, varargin )
...
for dataCol = psdsData_col
    ...
    grid on
    if noCase ==1
        title('Branch Reactive Power Flow Difference')
    else
        title({'Branch Reactive Power Flow Difference'; ['Case: ', LTDCaseName]})
    end
    xlabel('Time [sec]')
    ylabel('Power [MVAR]')
    ...
    if printFigs
        ...
        export_fig([LTDCaseName,'Qbr2'],'-pdf'); % to print fig
    end
    %% end of function
end
