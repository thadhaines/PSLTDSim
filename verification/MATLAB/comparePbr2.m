function [  ] = comparePbr2( mir, psds_data, varargin )
...
for dataCol = psdsData_col
    ...
    %if (str2num(FbusSTR) == 79) && (str2num(TbusSTR) == 7 )
    %    fprintf("DEBUG STOP\n")
    %end
    ...
    if abs(mean(cData))>100
        fprintf("Average more than 100: %s : %d\n" , ltdDataName,mean(cData) );
    end
    ...
    grid on
    if noCase ==1
        title('Branch Real Power Flow Difference')
    else
        title({'Branch Real Power Flow Difference'; ['Case: ', LTDCaseName]})
    end
    xlabel('Time [sec]')
    ylabel('Power [MW]')
    ...
    if printFigs
        ...
        export_fig([LTDCaseName,'Pbr2'],'-pdf'); % to print fig
    end
    %% end of function
end
