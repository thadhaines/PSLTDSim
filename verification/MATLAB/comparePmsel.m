function [  ] = comparePmsel( mir, psds_data, varargin )
...
for area = 1:max(size(mir.areaN)) % for each area
    ...
    for slack = 1:max(size(mir.(curArea).slackBusN))
        ...
        stairs(mir.t, mir.(curArea).(curSlack).S1.Pm,'-o','color',ltdColors{linesP+1},'linewidth',1)
    ...
    for gen = 1:max(size(uniueGens))
        ...
        for GenId = 1:mir.(curArea).(curGenBus).Ngen
            ...
            if GenId > 1
                ...
                stairs(mir.t, mir.(curArea).(curGenBus).(curGen).Pm,'-o','color',ltdColors{linesP+1},'linewidth',1)
                linesP = mod(linesP+1 , size(ltdColors,1));
            else
...
if ismember(busNum,sel)
    ...
    if dupe == 1
        ...
        plot(t, psds_data.Data(:,pm_col(curCol)),'-','color',ltdColors{linesP+1},'linewidth',1.5)
        linesP = mod(linesP+1 , size(ltdColors,1));
    else
       % plot(t, psds_data.Data(:,pm_col(curCol)))
        plot(t, psds_data.Data(:,pm_col(curCol)),'-','color',ltdColors{linesP+1},'linewidth',1.5)
...
if printFigs
    ...
    export_fig([LTDCaseName,'Pmsel'],'-pdf'); % to print fig
end
