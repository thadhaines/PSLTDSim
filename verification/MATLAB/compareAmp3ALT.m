function [  ] = compareQbr3( mir, psds_data, varargin )
...
for dataCol = psdsData_col
    ...
    if abs(mean(LTDdata)) < 1.0
        fprintf("Skipping Zero data in %s\n",ltdDataName)
        %continue
    end
    %   compare data
...
grid on
if noCase ==1
    title('Branch Current Flow Percent Difference')
else
    title({'Branch Current Flow Percent Difference'; ['Case: ', LTDCaseName]})
end
xlabel('Time [sec]')
...
if printFigs
    ...
    export_fig([LTDCaseName,'Amp3ALT'],'-pdf'); % to print fig
end
%% end of function
end
