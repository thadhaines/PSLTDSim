/* Kundur ramp EPCL */
/* Open .sav file */
@ret = getf("C:\LTD\pslf_systems\kundur4LTD\kundur4LTD1a.sav")

/* MAKE SURE BASE CASE IS SOLVED. */
@ret = soln()

/* READ THE DYNAMIC DATA. */
@ret = rdyd("C:\LTD\pslf_systems\kundur4LTD\kundur4LTD.dyd","",1)

/* INIT THE DYNAMIC DATA AND SET THE CHANNEL FILE. */
@ret = init("C:\LTD\pslf_systems\kundur4LTD\results\kundur.ramp01a.chf", "C:\LTD\pslf_systems\kundur4LTD\results\kundur.ramp01a.rep", 0, 1, "C:\LTD\inrun\kr0.p", "", 0)
dypar[0].nplot   = 1     
dypar[0].nscreen = 5   

/* Tic */
@ret = tictoc(0)

/* RUN sim for 15 SEC to . */
@tpause = 180.0
dypar[0].tpause  = @tpause  
@ret = run()     

/*  CLOSE THE CHANNEL FILE. */
@ret = dsst()

/* Toc */
@time = tictoc(1)
logterm("Elapse time is ",@time,"<")

end