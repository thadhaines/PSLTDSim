...
\chead{Long Term Dynamic Simulation Thoughts \\ January 21, 2019}
\lhead{Research \\ }
% spacing
\headheight 29 pt
\headsep 6 pt
...
General Electric's Positive Sequence Load Flow (PSLF) is a commercial power systems analysis software package that is regarded as an industry standard. Many working and accepted system models exist that are compatible with PSLF. Additionally, GE has created a Python interface to their software for a more customized experimentation and simulation process. However, the Python code support is not completely functional at this time (dynamic simulation is not fully supported). %Luckily, there seems to be enough functionality to perform LTD. \\
