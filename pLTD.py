...
for area in mir.Area:
    ...
    for gen in area.Machines:
        if gen.gov_model:
            ...
            ax.plot(mins, np.array(gen.gov_model.r_x1)/normVal*100, linestyle = '-',linewidth=1,
                    label = 'Gen ' + str(gen.Busnum)+r'. Travel$_{total}$: '+str(round(gen.gov_model.totValveMovement,3)*100)+'%'  )
    ...
    ax.set_title(r'Area '+str(gen.Area)+' Valve Movement \n Case: ' + caseName)
    ...
    if printFigs: plt.savefig(caseName+str(gen.Area)+'ValveMov.pdf', dpi=300)
